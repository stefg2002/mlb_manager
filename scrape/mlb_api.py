...
def get_player_links(teams):
    ...
    for team in teams:
        for player in team['Roster']:
            links.append({'Link': f'/api/v1/people/{player}', 'Team Name': team['Club Name']})
    return links
...
def scrape():
    ...
    with requests.Session() as session:
        ...
        for player in links:
            ...
            team_name = player['Team Name'].lower()
            players.append({'first_name': first_name, 'last_name': last_name, 'normalized_name': normalized_name, 'age': age, 'position': position, 'bats': bats, 'throws': throws, 'mlb_id': mlb_id, 'team_name': team_name})
